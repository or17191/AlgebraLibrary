...
T dot(const matrix<T>& a, const matrix<T>& b){
	if(a.columns() != b.columns() || a.rows() != b.rows()) throw std::invalid_argument("matrixes must have same shapes");
	T res = 0;
...
template<typename T>
size_t matrix<T>::rows() const { return _rows; }


template<typename T>
size_t matrix<T>::columns() const { return _columns; }
...
template<typename T>
void matrix<T>::gaussian_elimination() {
...
T matrix<T>::det() const{
	...
	if(is_upper_triangular() || is_lower_triangular()){
		T res = 1;
		for(int i = 0; i < _rows; ++i) res *= self(i,i);
		return res;
	} 
